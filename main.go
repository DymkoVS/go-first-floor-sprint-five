...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	s1 := CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift
	t1 := r.Duration.Hours() * MinInHours
	return s1 * r.Weight / MInKm * t1
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	s1 := CaloriesWeightMultiplier * w.Weight
	s2 := math.Pow(w.meanSpeed()*KmHInMsec, 2)
	s3 := CaloriesSpeedHeightMultiplier * w.Weight
	s4 := w.Duration.Hours() * MinInHours
	return s1 + (s2/(w.Height/100))*s3*s4
}
